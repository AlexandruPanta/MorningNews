...
function Header() {
	...
	const handleRegister = () => {
		fetch('https://morning-news-backend-ashen.vercel.app/users/signup', {
			method: 'POST',
	...
	const handleConnection = () => {
		...
		fetch('https://morning-news-backend-ashen.vercel.app/users/signin', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
	...
	if (user.token) {
		userSection = (
			<div className={styles.logoutSection}>
				...
				<FontAwesomeIcon onClick={() => hadnlehidden()} icon={faEye} className={styles.userSection}/>
			</div>
		);
	} else {
		if (isModalVisible) {
